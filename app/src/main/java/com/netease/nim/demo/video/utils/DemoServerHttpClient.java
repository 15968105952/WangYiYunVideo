package com.netease.nim.demo.video.utils;
...
import com.netease.nim.demo.entity.TranscodeResponseEntity;
import com.netease.nim.demo.modle.VideoInfoEntity;
import com.netease.nim.demo.video.DemoServersVideo;
...
//        body.put(REQUEST_SID, DemoCache.getSid());
        body.put(REQUEST_SID, "1824902907");
        body.put(VID, vid);
...
//        body.put(REQUEST_SID, DemoCache.getSid());
    ...
    public void videoInfoGet(@Nullable Long vid, int type, final DemoServerHttpCallback<List<VideoInfoEntity>> callback){
        String url = DemoServersVideo.apiServer() + API_VIDEO_INFO_GET;
        Map<String, String> headers = new HashMap<>(1);
        headers.put(HEADER_CONTENT_TYPE, "application/x-www-form-urlencoded; charset=utf-8");
        headers.put(HEADER_DEMO_ID, HEADER_DEMO_ID_VALUE);
